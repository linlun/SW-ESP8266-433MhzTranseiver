...
int8 parseSIRC(const uint16 *buf, uint8 len, Ir_Protocol_Data_t *proto) {
	...
	proto->protocol = IR_PROTOCOL_SIRC;
...
int8 parseRC5(const uint16 *buf, uint8 len, Ir_Protocol_Data_t *proto) {
	...
	proto->protocol=IR_PROTOCOL_RC5;
...
int8 expandRC5(uint16 *buf, uint8 *len, Ir_Protocol_Data_t *proto) {
	...
	uint16 rawMessage=proto->data & 0x3fff;
...
int8 parseSharp(const uint16 *buf, uint8 len, Ir_Protocol_Data_t *proto) {
	...
	proto->protocol=IR_PROTOCOL_SHARP;
...
int8 parseNEC(const uint16 *buf, uint8 len, Ir_Protocol_Data_t *proto) {
	...
	proto->protocol=IR_PROTOCOL_NEC;
...
int8 parseSamsung(const uint16 *buf, uint8 len, Ir_Protocol_Data_t *proto) {
	...
	proto->protocol=IR_PROTOCOL_SAMSUNG;
...
int8 parseMarantz(const uint16 *buf, uint8 len, Ir_Protocol_Data_t *proto) {
	...
	proto->protocol=IR_PROTOCOL_MARANTZ;
...
int8 parsePanasonic(const uint16 *buf, uint8 len, Ir_Protocol_Data_t *proto) {
	...
	proto->protocol=IR_PROTOCOL_PANASONIC;
...
#define SKYSHORT 1
	...
	proto->protocol = IR_PROTOCOL_SKY;
...
#define IROBOTSHORT 1
	...
	proto->protocol = IR_PROTOCOL_IROBOT;
...
int8 parseNexa1(const uint16 *buf, uint8 len, uint8 index, Ir_Protocol_Data_t *proto) {
	...
	proto->protocol=IR_PROTOCOL_NEXA;
...
/* Only check type if we have separate support for type 3 */
	...
	proto->protocol=IR_PROTOCOL_VIKING;
	proto->timeout=0;
	proto->data=rawbitsTemp;
...
int8 parseVikingT3(const uint16 *buf, uint8 len, uint8 index, Ir_Protocol_Data_t *proto)
{
	...
	uint8 i, i2;
	uint64 rawbitsTemp = 0;//0xffffffffffffffff;
	//uint8 rawbitsTempArr[6] = {0,0,0,0,0,0};
...
int8 parseVikingSteak(const uint16 *buf, uint8 len, uint8 index, Ir_Protocol_Data_t *proto)
{
	...
	proto->protocol=IR_PROTOCOL_VIKINGSTEAK;
...
int8 parseRubicson(const uint16 *buf, uint8 len, uint8 index, Ir_Protocol_Data_t *proto)
{
	...
	proto->protocol=IR_PROTOCOL_RUBICSON;
...
int8 parseOregon(const uint16 *buf, uint8 len, uint8 index, Ir_Protocol_Data_t *proto)
{
	...
	uint8 data[IR_OREGON_DATASIZE]; //Verifiera minsta möjliga storlek
	uint8 i2 = 0;
	uint8 b_i;
	uint8	currentBit = 1;
	uint8 done = 0;
	...
	uint16 temp =0;
	uint64 rawbitsTemp = 0;//0xffffffffffffffff;
	const uint8 nibbleSwap[16] = {0x0u,0x8u,0x4u,0xCu,0x2u,0xAu,0x6u,0xEu,0x1u,0x9u,0x5u,0xDu,0x3u,0xBu,0x7u,0xFu};
...
/*
	...
	*/
	//----------------------
	uint8 bitlenght = 0;
	uint16 sensorType = (uint16)rawbitsTemp;
	switch (sensorType) 
	{
	  case 0xf824:
	  case 0x1d20:
	...
	switch (sensorType) 
	{
	  ...
	  case 0xf8b4:
	  {
	    ...
	    * f   =  Channel
	    * -----------------------------*/
	    //printf("begi: %x %x %x %x\n", (uint16)(rawbitsTemp>>48), (uint16)(rawbitsTemp>>32), (uint16)(rawbitsTemp>>16), (uint16)rawbitsTemp);
	    
	    //Convert hunmidity to decimal from BCD
	    i = (uint8)( (rawbitsTemp & 0xFF ));
	    //printf("humi: %x\n", i);
	    i = ((uint8)nibbleSwap[i & 0xF]<<4) + ((uint8)(nibbleSwap[(i>>4) & 0xF])&0x0f);
	    //printf("humi: %x\n", i);
	    index = (i>>4)*10 + (i & 0x0Fu);
	    proto->data = index; // humidity
	    
	    //Convert temperature to decimal from BCD
	    temperature = (uint16)( ((rawbitsTemp >> 8) & 0x7FFF ));
	    //printf("temp: %x\n", temperature);
	    ...
	    proto->data += ((uint64)nibbleSwap[(uint8)( ((rawbitsTemp >> 36 ) & 0xF))]) << 46;
	    //printf("chan: %d\n", nibbleSwap[(uint8)( ((rawbitsTemp >> 36 ) & 0xF))]);
	    
	    //Add battery information
	    proto->data += ((uint64)(nibbleSwap[(uint8)( ((rawbitsTemp >> 24 ) & 0xF))])&0x1) << 45;
	    //printf("bat : %d\n", ((nibbleSwap[(uint8)( ((rawbitsTemp >> 24 ) & 0xF))])&0x1));
	    
	    //Add rolling code information
	    i = (uint8)((rawbitsTemp >> 28 ) & 0xFF);
	    i = (uint8)nibbleSwap[i & 0xF] + ((uint8)(nibbleSwap[(i>>4) & 0xF]<<4)&0xf0);
	    i = i & 0x1F;
	    proto->data += ((uint64)(i)) << 40;
	    //printf("roll : %d\n", i);
	    ...
	    * -----------------------------*/
	    ...
	    //printf("done: %x %x %x %x\n", (uint16)(proto->data>>48), (uint16)(proto->data>>32), (uint16)(proto->data>>16), (uint16)proto->data);
	  ...
	  case 0x2914:
	  {
	    ...
	    * -----------------------------*/
	    ...
	    temp = (uint16)( (rawbitsTemp & 0xFFFF ));
	    //printf("i/h : %x\n", temp);
	    ...
	    //Add channel information
	    proto->data += ((uint64)nibbleSwap[(uint8)( ((rawbitsTemp >> 52 ) & 0xF))]) << 46;
	    //printf("chan: %d\n", nibbleSwap[(uint8)( ((rawbitsTemp >> 52 ) & 0xF))]);
	    
	    //Add battery information
	    proto->data += ((uint64)(nibbleSwap[(uint8)( ((rawbitsTemp >> 40 ) & 0xF))])&0x1) << 45;
	    //printf("bat : %d\n", ((nibbleSwap[(uint8)( ((rawbitsTemp >> 40 ) & 0xF))])&0x1));
	    
	    //Add rolling code information
	    i = (uint8)((rawbitsTemp >> 44 ) & 0xFF);
	    i = (uint8)nibbleSwap[i & 0xF] + ((uint8)(nibbleSwap[(i>>4) & 0xF]<<4)&0xf0);
	    i = i & 0x1F;
	    proto->data += ((uint64)(i)) << 40;
	    //printf("roll : %d\n", i);
	  ...
	  case 0x1984:
	  {
	    ...
	    * -----------------------------*/
	    ...
	    temp = (uint16)( (rawbitsTemp & 0xFFF ));
	    //printf("w_av: %x\n", temp);
	    temp = (uint16)nibbleSwap[temp & 0xF] + ((uint16)(nibbleSwap[(temp>>4) & 0xF]<<4)&0xf0) + ((uint16)(nibbleSwap[(temp>>8) & 0xF]<<8)&0xf0);
	    //printf("humi: %x\n", temp);
	    temp = ((temp>>8) & 0x0Fu)*100 +((temp>>4) & 0x0Fu)*10 + (temp & 0x0Fu);
	    proto->data = temp; // wind average
	    
	    //Convert current wind to decimal from BCD
	    temp = (uint16)( ((rawbitsTemp >> 12) & 0xFFF ));
	    //printf("w_av: %x\n", temp);
	    temp = (uint16)nibbleSwap[temp & 0xF] + ((uint16)(nibbleSwap[(temp>>4) & 0xF]<<4)&0xf0) + ((uint16)(nibbleSwap[(temp>>8) & 0xF]<<8)&0xf0);
	    //printf("humi: %x\n", temp);
	    temp = ((temp>>8) & 0x0Fu)*100 +((temp>>4) & 0x0Fu)*10 + (temp & 0x0Fu);
	    ...
	    //store direction
	    i = (uint8)( ((rawbitsTemp >> 32) & 0xF ));
	    proto->data += (uint32_t)i << 24;

	    //Add channel information
	    proto->data += ((uint64)nibbleSwap[(uint8)( ((rawbitsTemp >> 48 ) & 0xF))]) << 46;
	    //printf("chan: %d\n", nibbleSwap[(uint8)( ((rawbitsTemp >> 36 ) & 0xF))]);
	    
	    //Add battery information
	    proto->data += ((uint64)(nibbleSwap[(uint8)( ((rawbitsTemp >> 36 ) & 0xF))])&0x1) << 45;
	    //printf("bat : %d\n", ((nibbleSwap[(uint8)( ((rawbitsTemp >> 24 ) & 0xF))])&0x1));
	    
	    //Add rolling code information
	    i = (uint8)((rawbitsTemp >> 36 ) & 0xFF);
	    i = (uint8)nibbleSwap[i & 0xF] + ((uint8)(nibbleSwap[(i>>4) & 0xF]<<4)&0xf0);
	    i = i & 0x1F;
	    proto->data += ((uint64)(i)) << 40;
	    //printf("roll : %d\n", i);
...
#if (IR_PROTOCOLS_USE_RUBICSONSTN)
/**
 * Test data on Rubicson weather station sensor protocol
 * 
 * 
 * 
 * @param buf
